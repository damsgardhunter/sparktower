...
| **Fund it** | Get the story, the numbers and the plan into a shape that gets backed | 4 weeks to in-market |
...
- **Systemize** — `restaurant` · `service` · `retail` · `other`
- **Fund it** — `startup_equity` · `local_community` · `loan_grant` · `other`

`other` triggers two setup questions from Novva: what does "done" look like for this project,
...
| Actor | Meaning |
| --- | --- |
| `novva-builds` | Novva produces working output. User runs or reviews it. |
| `novva-drafts` | Novva writes or generates. User edits or approves. |
| `user-decides` | Novva presents options with reasoning. User chooses. |
| `user-does` | Only a human can do this — direction, judgment, testing, talking to people. |

**No blank text fields anywhere.** Where input is needed, Novva generates two or three options
...
Below Verified, Novva leans on check-in narrative rather than presenting pace arithmetic with
...
active does not push the date out. Novva absorbs it against remaining work, notes the overrun,
...
3. **Injected** — project-specific tasks Novva adds, grounded in the user's actual artifacts.
   Capped at 2–3 per phase. If Novva can't name the artifact that motivated a task, it doesn't add it.
...
- **Novva panel** — what it just noticed, what it recalculated, one or two actions
- **Next action card** — one task, with what Novva already did and what's left for the user
...
**M1.1 — Product statement** · `novva-drafts` · 10m
Novva reads the setup description and generates three statements with different emphases — not
...
**M1.2 — The core loop** · `novva-drafts` · 20m
...
**M1.3 — Scope cut** · `novva-drafts` → `user-decides` · 25m
Novva generates the full feature list the vision implies, then splits it into **in the month**
...
than something Novva argues about.

*Done when the in-scope list projects inside the user's target date.*

**M1.4 — Stack decision** · `novva-drafts` · 5m
- `app` — native vs cross-platform. Novva names the store review timeline here, since it lands in
...
**M1.5 — Scaffold** · `novva-builds` · 30m
...
**M1.6 — Data model** · `novva-drafts` · 20m
Novva derives the schema from the core loop. User reviews entity names, since those leak into the
UI forever.
- `game` — game state and save structure. What persists between sessions.
- `website` — content model, if there's content. Skip if static.

**M1.7 — Loop step one** · `novva-builds` · 1–2h
First step of the core loop, running against real data.

**M1.8 — Deploy** · `novva-builds` · 30m
...
**M2.1–n — Loop steps** · `novva-builds` · 1–3h each
One milestone per step from M1.2. Novva builds against the schema; user runs it. Broken into
...
- `game` — one milestone per verb in the moment-to-moment loop (move, act, respond, feedback),
  then a **feel checkpoint** · `user-does`. Novva can build the mechanic; it cannot judge the feel.
...
**M2.y — Persistence and auth** · `novva-builds` · 1–2h
...
**B.1 — Choose what to build** · `novva-drafts` → `user-decides` · 20m
Novva reads the deferred roadmap and the core loop, and sorts deferred items into two groups:
...
confirmed loop, or does it begin its own?* Checkable, consistent between users, and Novva can
state the reason in a way that holds up.

Novva recommends from the first group. Everything stays pickable — it's their product — but the
...
usually enough without Novva saying anything.
...
**B.3–n — Build the selected loops** · `novva-builds` · 1–3h each
...
**M3.1 — Empty, loading, error states** · `novva-drafts` · 1h
Novva writes them all, flags any it wasn't sure about.
- `game` — failure and edge states: death, quit mid-action, bad input, empty save.
- `website` — 404, form failure, empty search.

**M3.2 — Visual pass** · `novva-builds` · 1–2h
Novva applies a coherent pass; user picks direction from two or three options rather than
describing what they want.
- `game` — **game feel** pass, distinct from visual polish and more important: hit feedback,
  timing, transitions, audio cues. Novva implements, user judges.

**M3.3 — Onboarding** · `novva-builds` · 1h
...
**M3.4 — Landing page** · `novva-drafts` · 45m
Headline from M1.1, three lines, one action.

**M3.5 — Analytics on the loop** · `novva-builds` · 30m
Loop starts and completions visible. Powers week 4.

**M3.6 — Feedback channel** · `novva-builds` · 15m
One click to reach the builder.

**M3.7 — Pricing** · `novva-drafts` · 20m
Novva proposes three models with reasoning. Free is a valid pick, offered as one rather than
...
- `game` — premium, free-to-play, or demo-plus-paid. **Timing conflict:** free-to-play changes
  what gets built, so for that model this decision belongs in week 1. Novva should raise
...
**M4.1 — The first ten** · `novva-drafts` → `user-does` · 30m
Novva drafts the list structure and outreach message. User supplies ten actual names — `user-does`
because only they know who these people are.
- `game` — ten playtesters, sourced from the genre's communities. Novva should say plainly that
...
No coaching. Note every hesitation. Novva provides the observation template and afterward turns
...
**M4.4 — Fix the top three frictions** · `novva-builds` · 2–4h

**M4.5 — Read the signal** · `novva-drafts` · 20m
Novva pulls analytics and reports what it means rather than asking the user to interpret.
...
Continue, adjust the loop, or move to another path. Novva makes the case for each based on what
...
handoff that Novva can draft, which is what makes the month realistic.

**Verification is Evidence tier**, not Verified — no codebase to check. Projections show as
ranges, and Novva leans harder on the weekly check-in narrative.

## Week 1 — See it clearly

**M1.1 — Time capture** · `user-does` · 10m/day
Novva prompts once daily and categorizes the response. Deliberately tiny — a full time audit is
the kind of homework that ends a path in week one.

**M1.2 — Only-me list** · `novva-drafts` · 20m
...
**M1.3 — Financial baseline** · `novva-drafts` · 30m
User connects an account or uploads statements; Novva reads and summarizes. No manual entry.

**M1.4 — Bottleneck ranking** · `novva-drafts` · 15m
...
Novva writes; the owner corrects. This is the week that would otherwise never happen, because
nobody writes their own SOPs.

**M2.1 — Delivery SOP** · `novva-drafts` · 1h
Novva interviews the owner conversationally — voice or chat — then writes the document.
...
**M2.2 — Intake and sales SOP** · `novva-drafts` · 1h

**M2.3 — Onboarding SOP** · `novva-drafts` · 45m
...
**M2.5 — Close the gaps** · `novva-drafts` · 30m

## Week 3 — Instrument and delegate

**M3.1 — Operating metrics** · `novva-drafts` · 20m
Novva proposes the three to five numbers that matter for this business shape.
...
**M3.2 — Where each number comes from** · `novva-drafts` · 20m
Source mapping. Novva pulls what it can automatically.

**M3.3 — Dashboard** · `novva-builds` · 1h

**M3.4 — Review cadence** · `user-does` · 5m
A recurring slot that exists in a calendar.

**M3.5 — Role definition** · `novva-drafts` · 30m
Built from the only-me list. What this person owns, what they decide, what escalates.

**M3.6 — Hire, contract, or automate** · `novva-drafts` → `user-decides` · 20m
Novva costs all three against the bottleneck ranking.

**M3.7 — Job post or automation spec** · `novva-drafts` · 30m
...
**M4.1 — Automate the top two repetitive tasks** · `novva-builds` · 2–3h
...
**M4.2 — Runbook for when automation breaks** · `novva-drafts` · 30m

**M4.3 — Pricing review** · `novva-drafts` · 30m
...
Novva should say the three-day version is a first proof, not the finish line.

**M4.5 — Gap list and fixes** · `novva-drafts` · 1h

**M4.6 — What's next** · `user-decides` · 10m
Deeper systemizing, or Fund it to finance growth.
...
the **methods**, and Novva usually stacks two or three toward the goal.
...
Novva never presents "you don't qualify" as an endpoint. Every situation has a live route.
...
strong the case is, and Novva's job is maximum case strength for minimum user effort.

**The default is that Novva gathers it.** The user should never be handed a research assignment
they could have paid someone to do.

**Tier 0 — Novva does it. Zero user effort.**
...
**Tier 1 — One afternoon. Calls and emails Novva drafts.**
Real rent quotes on two or three spaces. Supplier price sheet. Insurance quote. Permit costs from
the city. Novva writes the emails and call scripts; the user sends and dials.
*Highest score-per-hour in the system. Guessed becomes quoted.*

**Tier 2 — Something they'd enjoy doing anyway.**
For food: cook for twelve people, collect reactions on a one-page form Novva generates. That's a
dinner party, not a business operation, and it's the restaurant's version of a product demo.
Photos of the food. A one-page site with email signup that Novva builds.
...
insurance. Novva presents it as one option, names the actual requirements up front, and never
...
Novva builds it. The user answers questions conversationally and makes decisions; Novva writes
...
**2. Internal consistency** — do the numbers agree. Machine-checkable, immediately useful. Novva
...
**4. Assumption risk** — Novva names the two or three assumptions that break the model if wrong by
...
Novva research and makes a few phone calls lands in the mid-eighties — quoted costs, real
...
**M1.1 — Situation read** · `novva-drafts` · 20m
Conversational. What exists, money, credit, audience, timeline, shape.

**M1.2 — Route stack** · `novva-drafts` → `user-decides` · 20m
Novva proposes two or three routes toward the user's goal with the case for each.

**M1.3 — Tier 0 research** · `novva-builds` · 1h, mostly unattended
Novva gathers comparables, benchmarks, demographics, competitor pricing. User watches it populate.

**M1.4 — Plan v1 generated** · `novva-builds` · 1h
Full draft from the situation read plus tier 0 research. First score appears here.

**M1.5 — Financial model** · `novva-builds` · 1h
Driver-based. User adjusts drivers; Novva handles the math.

**M1.6 — Read the score** · `novva-drafts` · 15m
Sub-scores, the top three actions, and what each is worth.

## Week 2 — Evidence and materials

**M2.1 — Tier 1 outreach pack** · `novva-drafts` · 30m
...
**M2.3 — Plan updated with quotes** · `novva-builds` · auto
Guessed becomes quoted. Score moves. This is the moment the system proves itself.

**M2.4 — Assumption risk pass** · `novva-drafts` · 30m
Novva names what breaks the model and drafts the mitigation for each.

**M2.5 — Materials** · `novva-builds` · 2h
Route-dependent: deck for `equity`, application pack for `loan_grant`, campaign page and story for
`local_community`.

**M2.6 — The five hard questions** · `novva-drafts` · 30m
Generated from the actual weak spots in the numbers, not a generic list. Novva drafts answers;
user sharpens.

## Week 3 — Set up the route

**M3.1 — Showcase page** · `novva-builds` · 45m
...
**M3.2 — Route setup** · `novva-builds` · 1–2h
...
**M3.4 — Score check before going out** · `novva-drafts` · 15m
Last chance to raise it cheaply. Novva names anything still guessed that could be quoted today.
...
**M4.2 — Response log** · `novva-drafts` · ongoing
Novva structures notes after every meeting, call, or rejection.

**M4.3 — Objection tracking** · `novva-drafts` · ongoing
What keeps coming up.

**M4.4 — Revise from objections** · `novva-builds` · 1–2h
Materials and plan updated. Score recalculates.

**M4.5 — Pipeline view** · `novva-drafts` · 30m
...
Grant and loan applications have fixed dates, so this type inverts the model: Novva counts
...
- `SH-01` Positioning statement — Ship, Fund
- `SH-02` Pricing model — Ship, Systemize, Fund
- `SH-03` Core metric definition — all three
- `SH-04` Financial baseline — Systemize, Fund
- `SH-05` Customer list — Ship, Systemize
- `SH-06` Competitor landscape — Ship, Fund

## Typical routes through the tree

- **Idea → product → money:** Ship → Fund → Systemize
- **Existing business, owner drowning:** Systemize → Fund
- **Existing business, needs capital:** Fund → Systemize
- **No money, big idea:** Fund (plan + presales) → Ship → Systemize

Novva proposes the next path at each path's final milestone, with the case based on what actually
