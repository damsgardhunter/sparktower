...
Respond ONLY with valid JSON (no markdown, no code fences):
{
  ...
  app.post("/api/mock-interviews/:id/finish", isAuthenticated, rateLimit("ai"), async (req: any, res) => {
    // metering: free by design — the verdict for questions already paid for, generated once (a repeat returns it), on the AI burst limit; a failed call stores a plain verdict and charges nothing. Held to that by test/integration/ai-metering-sweep.test.ts ("the interview verdict"): the model runs once per interview and finishing again returns the stored verdict without calling it.
    try {
      const userId = req.user.id;
      const interview = await storage.getMockInterview(req.params.id);
      if (!interview || interview.userId !== userId) {
