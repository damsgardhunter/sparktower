...
If THE BUILDER'S OWN DOCS appear in the state, they are the primary source for loops: builders write down the loops they mean to build. Take the loops from there by name, then use the code and audit to judge each one's state. Only add a loop the docs don't mention when the code clearly shows a cycle the docs forgot.
...
Name each loop by what the user is doing (build, explore, back, sell), one line of steps, one user per loop. Keep loops SEPARATE: never fold two cycles for different users or motivations into one entry. Most products have two to four; report fewer rather than pad. State on the evidence — "built" (the cycle works end to end), "partly", or "planned". ${opts.knownLoops?.length ? `Loops already recorded (do not repeat them; you may report their state): ${opts.knownLoops.join("; ")}.` : ""}` : ""}
