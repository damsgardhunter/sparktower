...
describe("what a day is allowed to cost", () => {
  ...
  /*
   * Both of the tests skipped in this file describe the subscription model,
   * and pay-per-use replaced it: every tier is free, every tier draws the same
   * monthly allowance, and dollars are taken per outcome rather than per plan.
   *
   * That makes this sum unanswerable rather than failing. `DAILY_CREDIT_CAP`
   * still rises 10/25/60/120 across tiers that are now identical, so two of
   * its entries are larger than the whole month they are drawn from. The
   * server no longer enforces it for exactly that reason (see the ceilings
   * note in server/entitlements.ts) — the AI rate limit bounds the burst
   * instead.
   *
   * Left here rather than deleted: if a paid tier ever comes back, these are
   * the sums that have to be redone before it ships.
   */
  it.skip("never exceeds the month it is drawn from, for the tiers that have one", () => {
    for (const tier of TIER_IDS) {
...
describe("the actions whose cost is nothing like their price", () => {
  ...
  /*
   * `netOf(pro.priceMonthly)` is zero now, because Pro costs nothing — so a
   * month of audits can never come in under it and the check can only fail.
   * What bounds this spend today is the per-action ceiling above, which is
   * enforced, plus the price of the outcome itself.
   */
  it.skip("keeps a month of the dearest action inside the dearest plan's revenue", () => {
    /*
