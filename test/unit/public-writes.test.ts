...
const PUBLIC_WRITES: Record<string, string> = {
  ...
  "POST /api/problem-reports": "nothing — somebody saying a screen is broken, which is most worth hearing from the person who cannot sign in to say it; a message and a path with its query dropped, twelve per address every thirty minutes",
  "POST /api/client-errors": "nothing — a browser reporting that one of its own screens threw, which is most worth hearing when it happened before anybody could sign in; redacted, stored nowhere, ten per address every ten minutes",
