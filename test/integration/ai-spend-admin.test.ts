...
describe("the AI spend console", () => {
  it("answers on every screen, and counts what was actually spent", async () => {
    ...
    /*
     * Derived, not typed. The literal used to be 40, from a free month of
     * twenty credits; pay-per-use gave every tier the same twenty-five and the
     * percentage became 32, so the test failed on a number the route had got
     * right. The line above already checks the allowance itself, so working
     * this one out from the same constant keeps them from drifting apart
     * again.
     */
    expect(top.allowanceUsed, "eight credits of the free month").toBe(Math.round((8 / ENTITLEMENTS.free.credits) * 100));
