...
export function cleanNumbers(raw: unknown): { ok: true; numbers: CheckinNumbers } | { ok: false; message: string } {
  ...
  /*
   * Built from pairs rather than assigned key by key, because every key here
   * came from a client: `out[id] = n` with an id of `__proto__` sets this
   * object's prototype instead of recording a number, and `constructor`
   * shadows one. `Object.fromEntries` defines own properties, so neither can
   * reach the prototype however the name is spelled.
   *
   * Not `Object.create(null)`, which was the first attempt: an object with no
   * prototype reaches the Postgres driver on its way to jsonb, and the driver
   * asks every value for its `constructor`. Eight integration tests turned
   * into 500s that said "Cannot read properties of null".
   */
  ...
  for (const [k, v] of entries) {
    ...
    if (v === null || v === "" || v === undefined) { pairs.push([id, null]); continue; }
  ...
  return { ok: true, numbers: Object.fromEntries(pairs) as CheckinNumbers };
}
