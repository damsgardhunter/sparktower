...
- **Where the mapping lives is the catch.** A route can answer 502 for the empty answer it checks itself and still end its catch with a plain 500, so an unreadable answer thrown by a helper comes back as "something went wrong" — which is what `/api/chat`, `/api/projects/:id/nova-guide` and `/api/projects/:id/ai/summarize-progress` did. All three now pass their catch through `respondToAiError`. The route scan judges this by reading the catch blocks rather than looking for a mention anywhere in the body, which is how it read those three as handled (`test/unit/route-coverage.test.ts`). The common shape — parsing in an inner `try` that answers 502, with a plain 500 outside it for everything else — still counts as handled, because it is.
